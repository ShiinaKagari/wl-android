...
| 端 | 定义 | 职责边界 |
|---|---|---|
| **开发机** (dev machine) | 当前主开发环境（运行 agent 的机器），git 仓库在此维护 | **代码改动只允许在开发机上做**；所有提交与 push 发生在这里 |
| **测试后端** (test backend) | 安卓设备上的容器（Droidspaces `--name=arch`） | **除 App 测试外的一切内容**：server（wl-android）的构建、运行、日志、Plasma/KWin 会话、doctor、soak 等 |
| **测试前端** (test frontend) | 安卓机本身（设备 376627b8） | **只负责 App 的测试**：安装、启动、logcat 观察、触摸/键盘交互验证 |
